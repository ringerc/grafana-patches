...
// The behavior is gated by the `datasourceForwardHeaders` feature toggle;
// when the toggle is disabled the middleware is a no-op.
func NewForwardHeadersMiddleware(denyList []string, features featuremgmt.FeatureToggles) backend.HandlerMiddleware {
...
func (m *ForwardHeadersMiddleware) enabled() bool {
	return m.features != nil && m.features.IsEnabledGlobally(featuremgmt.FlagDatasourceForwardHeaders)
}

func (m *ForwardHeadersMiddleware) applyHeaders(ctx context.Context, pCtx backend.PluginContext, req any) error {
	if !m.enabled() || req == nil {
		return nil
	}
